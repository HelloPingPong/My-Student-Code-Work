...
-- <> is a another form of the not equal operator
SELECT city_name
FROM city
WHERE state_abbreviation <> 'CA';
...
-- SELECT the same only where not in New England 
SELECT city_name, area, population
FROM city
...
-- Postgres specific: ILIKE is case in-sensitive
SELECT city_name 
FROM city WHERE city_name LIKE '%A%';

SELECT city_name 
FROM city WHERE city_name ILIKE '%A%';
...
-- Parks established between June 13, 1950 and November 17, 1970
SELECT *
FROM park
WHERE date_established BETWEEN '1950-06-13' AND '1970-11-17';
