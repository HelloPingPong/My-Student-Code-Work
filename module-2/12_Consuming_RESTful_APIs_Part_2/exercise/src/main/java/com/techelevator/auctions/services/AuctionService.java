...
public class AuctionService {
    ...
    public Auction add(Auction newAuction) {
        ...
        HttpEntity<Auction> httpRequest = new HttpEntity<Auction>(newAuction, headers);
        String url = API_BASE_URL;
        Auction returnedAuction = null;
        try {
            // postForObject( url, http entity with the header/body, data type of the expected response
            returnedAuction = restTemplate.postForObject(url, httpRequest, Auction.class);
    ...
    public boolean update(Auction updatedAuction) {
        ...
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        HttpEntity<Auction> httpRequest = new HttpEntity<Auction>(updatedAuction, headers);
        String url = API_BASE_URL;
        Auction returnedAuction = null;
        try {
            // postForObject( url, http entity with the header/body, data type of the expected response
            restTemplate.put(url + updatedAuction.getId(), httpRequest);
            return true;
        } catch (RestClientResponseException e) {
        } catch (ResourceAccessException e) {
        }
