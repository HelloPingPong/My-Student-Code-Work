...
public class Exercises {
	...
	public Map<String, Integer> robPeterToPayPaul(Map<String, Integer> peterPaul) {
		/*
			if get( Peter ) > 0 && if get( Paul ) < 1000
				calculate half of Peter's money
	...
	public Map<String, Integer> consolidateInventory(Map<String, Integer> mainWarehouse,
		...
		/*
			pick one warehouse for example main
			loop through the other (remote)
			for each key in remote
				if exists in main then update that key in main to be sum of that key's value in both
				if does not exist in main then add it with the value from remote
			return main
		 */
		return null;
	}
