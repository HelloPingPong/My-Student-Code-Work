...
public class Exercises {
	...
	public List<String> no4LetterWords(String[] stringArray) {
		...
		/*
			...
			1. Loop over stringArray
			2. For each word in the array
				if word equal to 4 characters
					if true -> remove it (ignore it so it is not in the new list)
	...
	public List<Double> arrayInt2ListDouble(int[] intArray) {
		/*
			1. Create new list of type Double
			2. Loop over intArray
			3. divide by 2
			4. add it to new list - converted as Doubles
			5. return result
		 */
		List<Double> doubles = new ArrayList<Double>();

		for (int nextNum : intArray) {
			double half = (double) nextNum / 2;
			doubles.add(half);
		}
