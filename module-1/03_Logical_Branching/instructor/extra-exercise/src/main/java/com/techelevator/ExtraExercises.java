...
public class ExtraExercises {
	...
	public boolean sleepIn(boolean weekday, boolean vacation) {
		...
		// Sleep in if vacation is true
		if (vacation) {
			return true;
		}
		// Don't sleep in if weekday is true and vacation if false
		return false;
...
//		return aSmile == bSmile;
	...
	public boolean cigarParty(int cigars, boolean isWeekend) {
		// not a weekend and cigars between 40 and 60 success
		if (!isWeekend && (cigars >= 40 && cigars <= 60)) {
			return true;
		}
		// if a weekend and cigars over 40 success
	...
	public int luckySum(int a, int b, int c) {
		//if no 13 for any numbers then count all
		//If there is a 13 then dont count that number, or the number to the right
		if (a != 13 && b != 13 && c != 13) {
			return a + b + c;
		}
