<template>
  <div class="main">
    ...
    <div class="well-display">
      <div class="well">
        <span class="amount" v-on:click="reviewFilter = 0">{{ averageRating }}</span>
        Average Rating
      </div>

      <div class="well">
        <span class="amount" v-on:click="reviewFilter = 1">{{ numberOfOneStarReviews }}</span>
        1 Star Review{{ numberOfOneStarReviews === 1 ? '' : 's' }}
      </div>

      <div class="well">
        <span class="amount" v-on:click="reviewFilter = 2">{{ numberOfTwoStarReviews }}</span>
        2 Star Review{{ numberOfTwoStarReviews === 1 ? '' : 's' }}
      </div>

      <div class="well">
        <span class="amount" v-on:click="reviewFilter = 3">{{ numberOfThreeStarReviews }}</span>
        3 Star Review{{ numberOfThreeStarReviews === 1 ? '' : 's' }}
      </div>

      <div class="well">
        <span class="amount" v-on:click="reviewFilter = 4">{{ numberOfFourStarReviews }}</span>
        4 Star Review{{ numberOfFourStarReviews === 1 ? '' : 's' }}
      </div>

      <div class="well">
        <span class="amount" v-on:click="reviewFilter = 5">{{ numberOfFiveStarReviews }}</span>
    ...
    <div v-if="filteredReviews.length === 0">
      <p>No {{ reviewFilter }} Star Reviews Found</p>
    </div>
  ...
  computed: {
    filteredReviews() {
      return this.reviews.filter( review => {
        return this.reviewFilter === 0 ? true : this.reviewFilter === review.rating;
      });
    },
    averageRating() {
